        ...
        <section class="row">
            ...
            <section class="main col-sm-9 offset-sm-3 col-md-10 offset-md-2 p-0">
                <section class="header-content justify-content-between align-items-baseline">
                    <section>
                        <p class="medium lh-1-5 mb-0">EDIT MAPPING FEE</p>
                    </section>
                </section>
                <section class="col-sm-6 offset-sm-3 p-5">
                    ...
                    @if (session('warning'))
                        ...
                        <br/>
                    @endif
                    <h5>Edit fee mapping</h5>
                    <h6 class="medium-small">Please insert with valid data.</h6>
                    <hr class="clearfix"/>
                    <form method="post" action="{{ route('mapping_fee.update', [$mapping_charge->id, $mapping_fee->id]) }}" accept-charset="utf-8" role="form">
                        {{ csrf_field() }}
                        {{ method_field('put') }}
                        ...
                        <fieldset class="form-group">
                            ...
                            <input id="remaining_amount" name="remaining_amount" class="form-control" value="{{ $mapping_charge->amount - $mapping_fees->sum('amount') - $mapping_fee->amount }}" type="number" min="0" max="999999" disabled />
                        ...
                        <fieldset class="form-group{{ $errors->has('service_id') ? ' has-danger' : '' }}">
                            ...
                            <select id="account_id" name="account_id" class="form-control w-100">
                                ...
                                <option value="{{ $account->id }}" {{ $mapping_fee->account_id == $account->id ? 'selected' : '' }}>
                                    {{ $account->number }}
                                    -
                        ...
                        <fieldset class="form-group{{ $errors->has('amount') ? ' has-danger' : '' }}">
                            ...
                            <input id="amount" name="amount" class="form-control" type="number" min="0" max="{{ $mapping_charge->amount - $mapping_fees->sum('amount') }}" value="{{ $mapping_fee->amount }}" />
