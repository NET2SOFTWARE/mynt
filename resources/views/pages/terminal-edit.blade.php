                ...
                <section class="row">
                    <section class="col-sm-8 offset-sm-2 py-3">
                        <section class="mt-3 mb-5">
                            <section class="mb-4">
                                <h4><small>Update terminal</small></h4>
                            ...
                            <form class="mb-3" method="post" action="{{ route('terminal.update', [$terminal->id]) }}" accept-charset="utf-8" role="form">
                                {{ csrf_field() }}
		                        {{ method_field('put') }}
                                <section class="card card-block p-5">
                                    ...
                                    <fieldset class="form-group{{ $errors->has('code') ? ' has-danger' : '' }}">
                                        ...
                                        <input id="code" name="code" class="form-control" value="{{ $terminal->code }}" type="number" min="0" maxlength="8" max="99999999" />
                                    ...
                                    <fieldset class="form-group{{ $errors->has('merchant_id') ? ' has-danger' : '' }}">
                                        ...
                                        <select id="merchant_id" name="merchant_id" class="form-control w-100">
                                            ...
                                            <option value="{{ $merchant->id }}" {{ $terminal->merchants()->first()['id'] == $merchant->id ? 'selected' : '' }}>{{ $merchant->name }}</option>
                                            @endforeach
                                        </select>
