        ...
        <section class="row">
            ...
            <section class="main col-sm-9 offset-sm-3 col-md-10 offset-md-2 p-0">
                <section class="header-content justify-content-between align-items-baseline">
                    <section>
                        <p class="medium lh-1-5 mb-0">DEACTIVATE MEMBER</p>
                    </section>
                    <section class="d-flex justify-content-between align-content-st align-items-baseline">
                        <section class="d-flex justify-content-between align-content-start">
                            <section class="btn-group btn-group-sm">
                                <a href="{{ route('member.deactivate') }}" class="btn btn-secondary">Refresh</a>
                            </section>
                        </section>
                        <form action="{{ route('member.sort.deactivate') }}" method="POST" class="input-group input-group-sm ml-3" role="form">
                            {{ csrf_field() }}
                    ...
                    <section class="table-sm table-responsive medium p-3">
                        <table class="table">
                            ...
                            @foreach($members as $member)
                                <tr>
                                    <td>
                                        <span class="badge badge-{{ count($member->users) < 1 ? 'danger' : 'success' }} small-caps">
                                            {{ count($member->users) < 1 ? 'in' : '' }}active
                                        </span>
                                    </td>
                                    ...
                                    <td style="width:104px">
                                        <section class="btn-group btn-group-sm">
                                            <a class="btn btn-secondary small-caps py-0" href="{{ route('member.deactivate.process', [$member->id]) }}">
                                                {{ count($member->users) > 0 ? 'de' : '' }}activate
                                            </a>
                                        </section>
                                    </td>
                                </tr>
