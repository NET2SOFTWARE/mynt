                    ...
                    <section class="d-flex justify-content-between align-content-st align-items-baseline">
                        <section class="d-flex justify-content-between align-content-start">
                            <section class="btn-group btn-group-sm mr-3">
                                <a href="{{ route('product.sales.create') }}" class="btn btn-secondary">New</a>
                            </section>
                            <section class="btn-group btn-group-sm">
                                <a href="{{ route('product.sales.index') }}" class="btn btn-secondary">Refresh</a>
                            </section>
                        </section>
                        <form action="{{ route('product.sales.sort') }}" method="POST" class="input-group input-group-sm ml-3">
                            {{ csrf_field() }}
                                ...
                                <tr>
                                    ...
                                    <td class="text-right">
                                        <div class="btn-group btn-group-sm" role="group" aria-label="Tools">
                                            <a href="{{ route('product.sales.edit', [$row->id]) }}"
                                            ...
                                            <form id="product-delete-{{$row->id}}"
                                                action="{{ route('product.sales.delete', [$row->id]) }}"
