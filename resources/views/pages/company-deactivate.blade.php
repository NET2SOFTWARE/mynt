        ...
        <section class="row">
            ...
            <section class="main col-sm-9 offset-sm-3 col-md-10 offset-md-2 p-0">
                <section class="header-content justify-content-between align-items-baseline">
                    <section>
                        <p class="medium lh-1-5 mb-0">DEACTIVATE COMPANY</p>
                    </section>
                    <section class="d-flex justify-content-between align-content-st align-items-baseline">
                        <section class="d-flex justify-content-between align-content-start">
                            <section class="btn-group btn-group-sm">
                                <a href="{{ route('company.index', ['deactivate']) }}" class="btn btn-secondary">Refresh</a>
                            </section>
                        </section>
                        <form action="{{ route('company.sort.index') }}" method="POST" class="input-group input-group-sm ml-3" role="form">
                            ...
                            <section class="input-group input-group-sm">
                                ...
                                <input name="page" type="hidden" value="pages.company-deactivate" />
                        ...
                        <table class="table">
                            ...
                            @foreach($companies as $company)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        {{ $company->accounts()->first()['number'] }}
                                    </td>
                                    <td>
                                        <span class="badge badge-default small-caps">{{ strtolower($company->industry->name) }}</span></td>
                                    <td>
                                        @foreach($company->partnerships as $partnership)
                                        <span class="badge badge-info small-caps">{{ strtolower($partnership->name) }}</span>
                                        @endforeach
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group btn-group-sm" role="group" aria-label="Tools">
                                            <a href="#" 
                                                ...
                                                onclick="event.preventDefault();document.getElementById('company-deactivate-{{$company->id}}').submit();">
                                                {{ sizeof($company->users()->get()) < 1 ? '' : 'de' }}activate
                                            </a>
                                            <form id="company-deactivate-{{$company->id}}" action="{{ route('company.deactivate', [$company->id]) }}" method="POST" class="sr-only">
                                                {{ csrf_field() }}
                                                {{ method_field('delete') }}
                                            </form>
                                        </div>
