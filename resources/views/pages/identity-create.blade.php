                ...
                <section class="row">
                    <section class="col-sm-8 offset-sm-2 py-3">
                        <section class="mt-3 mb-5">
                            <section class="mb-4">
                                <h4><small>Create new identity</small></h4>
                            ...
                            <form class="mb-3" method="post" action="{{ route('identity.store') }}" accept-charset="utf-8" role="form">
                                {{ csrf_field() }}
                                <section class="card card-block p-5">
                                    ...
                                    <fieldset class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                        ...
                                        <input id="name" name="name" class="form-control" value="{{ old('name')  }}" placeholder="Identity name" type="text" aria-describedby="nameHelp">
                                        @if ($errors->has('name'))<section class="form-control-feedback">{{ $errors->first('name') }}</section>@endif
                                        <small class="form-text text-muted d-flex justify-content-between" id="nameHelp">Full identity name. Max. 40 characters, eg. `Identity Card` <span class="text-grey">Required</span></small>
                                    </fieldset>
                                </section>
