        ...
        <section class="row">
            ...
            <section class="main col-sm-9 offset-sm-3 col-md-10 offset-md-2 p-0">
                ...
                <section class="col-sm-6 offset-sm-3 p-5">
                    ...
                    <form method="post" action="{{ route('service.update', [$service->id]) }}" accept-charset="utf-8" role="form">
                        {{ csrf_field() }}
                        {{ method_field('put') }}

                        <fieldset class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                            <label for="name">Name</label>
                            <input id="name" name="name" class="form-control" value="{{ $service->name  }}" placeholder="service name" type="text" />
                            @if ($errors->has('name'))
                            ...
                            <small class="form-text text-muted d-flex justify-content-between">
                                Aplhanumeric, unique, eg. `Transfer`. <span class="text-grey">Required</span>
                            </small>
                        </fieldset>
                        <a href="{{ route('service.index') }}" tabindex="-1" class="btn btn-secondary small-caps">cancel</a>
