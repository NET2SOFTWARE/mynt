                    ...
                    <section class="d-flex justify-content-between align-content-st align-items-baseline">
                        <section class="d-flex justify-content-between align-content-start">
                            <section class="btn-group btn-group-sm mr-3">
                                <a href="{{ route('product.mapping_price.create') }}" class="btn btn-secondary">New</a>
                            </section>
                    ...
                    <section class="table-sm table-responsive medium p-3">
                        <table class="table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        {{ $product->{str_plural($product->account_type()->first()['name'])}->first()->products()->first()['name'] }}
                                    </td>
                                    <td>
                                        {{ sprintf('Rp %s', number_format($product->{str_plural($product->account_type()->first()['name'])}->first()->products()->withPivot('price')->first()->pivot->price)) }}
                                    </td>
                                    <td>
                                        <span class="badge badge-default small-caps">
                                        {{ $product->account_type()->first()['name'] }}
                                        </span> 
                                        {{ $product->companies()->first()['name'] }}
                                        {{ $product->merchants()->first()['name'] }}
                                    </td>
                                    <td>{{ $product->created_at->format('j F Y') }}</td>
                                    <td class="text-right">
                                        <div class="btn-group btn-group-sm" role="group" aria-label="Tools">
                                            <a href="{{ route('product.edit', [$product->{str_plural($product->account_type()->first()['name'])}->first()->products()->first()['id']]) }}"
                                                class="btn btn-secondary py-0 small-caps">
                                                edit
                                            </a>
                                            <a href="#"
                                                onclick="event.preventDefault();document.getElementById('product-delete-{{$product->{str_plural($product->account_type()->first()['name'])}->first()->products()->first()['id']}}').submit();"
                                                class="btn btn-secondary py-0 small-caps">
                                                delete
                                            </a>
                                            <form id="product-delete-{{$product->{str_plural($product->account_type()->first()['name'])}->first()->products()->first()['id']}}"
                                                action="{{ route('product.delete', [$product->{str_plural($product->account_type()->first()['name'])}->first()->products()->first()['id']]) }}"
