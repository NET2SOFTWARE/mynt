                ...
                <section class="card" style="min-height:540px">
                    <section class="card-header">
                        <ul class="nav nav-tabs card-header-tabs medium-small">
                            <li class="nav-item"><a class="nav-link active" href="{{ route('merchant.transaction.account') }}">Transfer To Account</a></li>
                                ...
                                <section class="form-group row{{ $errors->has('token') ? ' has-danger' : '' }}">
                                    ...
                                    <section class="col-sm-8">
                                        <section class="d-flex justify-content-between">
                                            ...
                                            <form id="generate-token" action="{{ route('generate.token', [Auth::user()->merchants->first()['accounts'][0]['number']]) }}" method="POST" style="display: none;">
                                                {{ csrf_field() }}
                                                {{ method_field('patch') }}
                                            </form>
                                        </section>
