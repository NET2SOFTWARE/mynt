    ...
    <article class="col d-flex justify-content-center">
        <section class="align-self-center con-sign-in">
            ...
            <section class="col-md-12 py-3">
                <form class="form-signin" method="POST" action="{{ route('post.login.member.company') }}" role="form" accept-charset="utf-8">
                    {{ csrf_field() }}
                    ...
                    <fieldset class="form-group{{ $errors->has('code') ? ' has-danger' : '' }}">
                        <label for="code" class="sr-only">Referral code</label>
                        <select id="code" name="code" class="custom-select w-100" style="padding-top:8px;padding-bottom:8px;height:42px">
                            <option selected disabled>Referral code</option>
                            @foreach($companies as $company)
                                <option value="{{ $company->code }}">{{ $company->code.' - '.$company->name }}</option>
                            @endforeach
                        </select>
                        @if ($errors->has('code'))
                            <section class="form-control-feedback">{{ $errors->first('code') }}</section>
                        @endif
                    </fieldset>
