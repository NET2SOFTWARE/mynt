        ...
        <section class="row">
            ...
            <section class="col-md-9 py-3">
                <h5 class="mb-4 d-flex flex-row justify-content-between align-items-baseline">
                    Transaction
                    <span>
                        @if(count(Auth::user()->members->first()['profiles']) < 1 && (!Auth::user()->isChildAccount()))
                ...
                <section class="card my-3" style="min-height:560px">
                    <section class="card-header">
                        <ul class="nav nav-tabs card-header-tabs medium-small">
                            ...
                            @endif
                            <li class="nav-item"><a class="nav-link active" href="{{ route('member.transactions.redeem') }}">Reedem</a></li>
                        </ul>
                    </section>
                    <section class="card-block">
                        @if (session('warning'))
                            <section class="alert small alert-warning mb-3 lh-1-2">{{ session('warning') }}</section>
                        @elseif(session('success'))
                            <section class="alert small mb-3 alert-success lh-1-2">{{ session('success') }}</section>
                        @endif
                        <section class="col-md-8 offset-md-2">
                            <form class="py-4" accept-charset="utf-8" role="form">
                                ...
                                <section class="form-group row">
                                    ...
                                    <section class="col-sm-8">
                                        <select id="bank" name="bank" class="custom-select w-100" aria-describedby="bankHelp">
                                            <option selected disabled>Bank account</option>
                                            @if(count(Auth::user()->members->first()['banks']) >= 1)
                                                <option value="{{ Auth::user()->members->first()['banks'][0]['pivot']['account_number'] }}" selected>{{ Auth::user()->members->first()['banks'][0]['bank_code'].'&nbsp;&nbsp;'.strtoupper(Auth::user()->members->first()['banks'][0]['bank_name']).'&nbsp;&nbsp;'.Auth::user()->members->first()['banks'][0]['pivot']['account_number'] }}</option>
                                            @endif
                                        </select>
                                        <small class="form-text small text-muted d-flex justify-content-between" id="bankHelp"><span>If the bank account is not yet registered <a href="{{ route('member.management.bank') }}" class="text-primary">register now</a></span><span class="text-grey ml-3">Required</span></small>
                                    </section>
                                </section>
