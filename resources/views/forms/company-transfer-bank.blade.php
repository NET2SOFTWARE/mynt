    ...
    <section class="col-sm-8">
        <select id="bank" name="bank" class="custom-select w-100" aria-describedby="bankHelp">
            ...
            @if(count(Auth::user()->companies->first()['banks']) >= 1)
                <option value="{{ Auth::user()->companies->first()['banks'][0]['pivot']['account_number'] }}" selected>{{ Auth::user()->companies->first()['banks'][0]['bank_code'].'&nbsp;&nbsp;'.strtoupper(Auth::user()->companies->first()['banks'][0]['bank_name']).'&nbsp;&nbsp;'.Auth::user()->companies->first()['banks'][0]['pivot']['account_number'] }}</option>
            @endif
        </select>
