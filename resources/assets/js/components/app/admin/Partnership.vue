    ...
    <transition name="fade">
        <section v-if="page_show" class="main col-sm-9 offset-sm-3 col-md-10 offset-md-2 p-0">
            <section class="header-content justify-content-between align-items-baseline">
                <section>
                    <p class="medium lh-1-5 mb-0">DATA-PARTNERSHIP</p>
                </section>
                <section class="d-flex justify-content-between align-content-st align-items-baseline">
                    <section class="d-flex justify-content-between align-content-start">
                        ...
                        <section class="btn-group btn-group-sm">
                            ...
                            <button v-on:click="show = !show; getPartnerships()" type="button" class="btn btn-secondary">Refresh</button>
                        </section>
                    </section>
                    ...
                    <section class="btn-group btn-group-sm ml-3">
                        <button v-on:click="fetchPartnerships(pagination.prev_page_url); show = !show" :disabled="!pagination.prev_page_url" type="button" class="btn btn-secondary">Previous</button>
                        <button v-on:click="fetchPartnerships(pagination.next_page_url); show = !show" :disabled="!pagination.next_page_url" type="button" class="btn btn-secondary">Next</button>
                    </section>
                </section>
            </section>
            <transition name="fade">
                <section class="table-sm table-responsive medium p-3" v-if="partnerships.length > 0 && show">
                    <table class="table">
                        <thead>
                        <tr class="medium">
                            <th>ID</th>
                        ...
                        <tbody>
                        <tr v-for="partnership in partnerships">
        ...
        methods: {
            ...
            getPartnerships() {
                axios.get('/api/partnership')
                    .then((response) => {
                        this.partnerships = response.data.partnerships.data;
                        this.makePagination(response.data.partnerships);
                        this.show = true;
                    });
            },

            fetchPartnerships(page_url) {
                let vm = this;
                page_url = page_url || '/api/partnership';
                axios.get(page_url)
                    .then((response) => {
                        vm.makePagination(response.data.partnerships);
                        vm.partnerships = response.data.partnerships.data;
                        vm.show = true;
                    });
            },
