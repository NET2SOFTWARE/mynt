    ...
    <transition name="fade">
        <section v-if="page_show" class="main col-sm-9 offset-sm-3 col-md-10 offset-md-2 p-0">
            <section class="header-content justify-content-between align-items-baseline">
                <section>
                    <p class="medium lh-1-5 mb-0">SUCCESS-TRANSACTION</p>
                </section>
                <section class="d-flex justify-content-between align-content-st align-items-baseline">
                    <section class="d-flex justify-content-between align-content-start">
                        ...
                        <section class="btn-group btn-group-sm">
                            ...
                            <button v-on:click="show = !show; getTransactions()" type="button" class="btn btn-secondary">Refresh</button>
                        </section>
                    </section>
                    ...
                    <section class="btn-group btn-group-sm ml-3">
                        <button v-on:click="fetchTransactions(pagination.prev_page_url); show = !show" :disabled="!pagination.prev_page_url" type="button" class="btn btn-secondary">Previous</button>
                        <button v-on:click="fetchTransactions(pagination.next_page_url); show = !show" :disabled="!pagination.next_page_url" type="button" class="btn btn-secondary">Next</button>
                    </section>
                </section>
            </section>
            <transition name="fade">
                <section class="table-sm table-responsive medium p-3" v-if="transactions.length > 0 && show">
                    <table class="table">
                        <thead>
                        <tr class="medium">
                            <th>ID</th>
                        ...
                        <tr v-for="transaction in transactions">
                            ...
                            <td>{{ transaction.state.name }}</td>
                            <td>{{ transaction.state.country.name }}</td>
                        </tr>
                        </tbody>
                    </table>
                </section>
    ...
    export default {
        ...
        methods: {
            ...
            fetchTransactions(page_url) {
                ...
                axios.get(page_url)
                    .then((response) => {
                        vm.makePagination(response.data.transactions);
                        vm.transactions = response.data.transactions.data;
                        vm.show = true;
                    });
            },
