                ...
                <section class="d-flex justify-content-between align-content-st align-items-baseline">
                    <section class="d-flex justify-content-between align-content-start">
                        ...
                        <section class="btn-group btn-group-sm">
                            ...
                            <button v-on:click="show = !show; getIdentities()" type="button" class="btn btn-secondary">Refresh</button>
                        </section>
                    </section>
                    ...
                    <section class="btn-group btn-group-sm ml-3">
                        <button v-on:click="fetchIdentities(pagination.prev_page_url); show = !show" :disabled="!pagination.prev_page_url" type="button" class="btn btn-secondary">Previous</button>
                        <button v-on:click="fetchIdentities(pagination.next_page_url); show = !show" :disabled="!pagination.next_page_url" type="button" class="btn btn-secondary">Next</button>
                    </section>
                </section>
            </section>
            <transition name="fade">
                <section class="table-sm table-responsive medium p-3" v-if="identities.length > 0 && show">
                    <table class="table">
                        <thead>
                        <tr class="medium">
                            <th>ID</th>
        ...
        methods: {
            ...
            getIdentities() {
                axios.get('/api/identity')
                    .then((response) => {
                        this.identities = response.data.identities.data;
                        this.makePagination(response.data.identities);
                        this.show = true;
                    });
            },

            fetchIdentities(page_url) {
                let vm = this;
                page_url = page_url || '/api/identity';
                axios.get(page_url)
                    .then((response) => {
                        vm.makePagination(response.data.identities);
                        vm.identities = response.data.identities.data;
                        vm.show = true;
                    });
            },
