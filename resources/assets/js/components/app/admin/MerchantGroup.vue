                ...
                <section class="d-flex justify-content-between align-content-st align-items-baseline">
                    <section class="d-flex justify-content-between align-content-start">
                        ...
                        <section class="btn-group btn-group-sm">
                            ...
                            <button v-on:click="show = !show; getMerchants()" type="button" class="btn btn-secondary">Refresh</button>
                        </section>
                    </section>
                    ...
                    <section class="btn-group btn-group-sm ml-3">
                        <button v-on:click="fetchMerchants(pagination.prev_page_url); show = !show" :disabled="!pagination.prev_page_url" type="button" class="btn btn-secondary">Previous</button>
                        <button v-on:click="fetchMerchants(pagination.next_page_url); show = !show" :disabled="!pagination.next_page_url" type="button" class="btn btn-secondary">Next</button>
                    </section>
                </section>
            </section>
            <transition name="fade">
                <section class="table-sm table-responsive medium p-3" v-if="merchants.length > 0 && show">
                    <table class="table">
                        <thead>
                        <tr class="medium">
                            <th>ID</th>
                        ...
                        <tr v-for="merchant in merchants">
                            ...
                            <td>{{ merchant.state.country.name }}</td>
                        </tr>
                        </tbody>
                    </table>
                </section>
        ...
        methods: {
            ...
            getMerchants() {
                axios.get('/api/merchant')
                    .then((response) => {
                        this.merchants = response.data.merchants.data;
                        this.makePagination(response.data.merchants);
                        this.show = true;
                    });
            },

            fetchMerchants(page_url) {
                let vm = this;
                page_url = page_url || '/api/merchant';
                axios.get(page_url)
                    .then((response) => {
                        vm.makePagination(response.data.merchants);
                        vm.merchants = response.data.merchants.data;
                        vm.show = true;
                    });
            },
