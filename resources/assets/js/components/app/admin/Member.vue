                ...
                <section class="d-flex justify-content-between align-content-st align-items-baseline">
                    <section class="d-flex justify-content-between align-content-start">
                        ...
                        <section class="btn-group btn-group-sm">
                            ...
                            <button v-on:click="show = !show; getMembers()" type="button" class="btn btn-secondary">Refresh</button>
                        </section>
                    </section>
                    ...
                    <section class="btn-group btn-group-sm ml-3">
                        <button v-on:click="fetchMembers(pagination.prev_page_url); show = !show" :disabled="!pagination.prev_page_url" type="button" class="btn btn-secondary">Previous</button>
                        <button v-on:click="fetchMembers(pagination.next_page_url); show = !show" :disabled="!pagination.next_page_url" type="button" class="btn btn-secondary">Next</button>
                    </section>
                </section>
            </section>
            <transition name="fade">
                <section class="table-sm table-responsive medium p-3" v-if="members.length > 0 && show">
                    <table class="table">
                        <thead>
                        <tr class="medium">
                            <th>ID</th>
                        ...
                        <tr v-for="member in members">
                            ...
                            <td>{{ member.state.country.name }}</td>
                        </tr>
                        </tbody>
                    </table>
                </section>
    ...
    export default {
        ...
        methods: {
            ...
            getMembers() {
                axios.get('/api/member')
                    .then((response) => {
                        ...
                        this.makePagination(response.data.members);
                        this.show = true;
                    });
            },
            ...
            fetchMembers(page_url) {
                let vm = this;
                page_url = page_url || '/api/member';
                axios.get(page_url)
                    .then((response) => {
                        vm.makePagination(response.data.members);
                        vm.members = response.data.members.data;
                        vm.show = true;
                    });
            },
