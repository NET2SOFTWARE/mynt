                ...
                <section class="d-flex justify-content-between align-content-st align-items-baseline">
                    <section class="d-flex justify-content-between align-content-start">
                        ...
                        <section class="btn-group btn-group-sm">
                            ...
                            <button v-on:click="show = !show; getCompanies()" type="button" class="btn btn-secondary">Refresh</button>
                        </section>
                    </section>
                    ...
                    <section class="btn-group btn-group-sm ml-3">
                        <button v-on:click="fetchCompanies(pagination.prev_page_url); show = !show" :disabled="!pagination.prev_page_url" type="button" class="btn btn-secondary">Previous</button>
                        <button v-on:click="fetchCompanies(pagination.next_page_url); show = !show" :disabled="!pagination.next_page_url" type="button" class="btn btn-secondary">Next</button>
                    </section>
                </section>
            </section>
            <transition name="fade">
                <section class="table-sm table-responsive medium p-3" v-if="companies.length > 0 && show">
                    <table class="table">
                        <thead>
                        <tr class="medium">
                            <th>ID</th>
                        ...
                        <tr v-for="company in companies">
                            ...
                            <td>{{ company.state.country.name }}</td>
                        </tr>
                        </tbody>
                    </table>
                </section>
    ...
    export default {
        ...
        methods: {
            ...
            getCompanies() {
                axios.get('/api/company')
                    .then((response) => {
                        ...
                        this.makePagination(response.data.companies);
                        this.show = true;
                    });
            },
            ...
            fetchCompanies(page_url) {
                ...
                axios.get(page_url)
                    .then((response) => {
                        vm.makePagination(response.data.companies);
                        vm.companies = response.data.companies.data;
                        vm.show = true;
                    });
            },
