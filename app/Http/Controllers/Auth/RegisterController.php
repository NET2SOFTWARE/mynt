...
class RegisterController extends Controller
{
    ...
    {
        return User::create([
            'name' => strtolower($data['first_name']) . ' ' . strtolower($data['last_name']),
    ...
    public function register(Request $request)
    {
        ...
        if ($this->user->attemptUniqueCredential($request->input('email'), $request->input('password'))) {
            ...
            return back()
                ...
                ->with('warning', 'Please use another account credential,we can not create your account at this moment.');
        }
        ...
        if (!$member = $this->member->save(['name' => $name, 'email' => $email, 'phone' => $phone]))
            return redirect()->back()
                            ->withInput($request->except(['password']))
                            ->with('warning', 'We are having problems now,please come back later');
